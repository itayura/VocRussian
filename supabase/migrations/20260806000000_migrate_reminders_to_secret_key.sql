-- Migrate scheduled reminders from the legacy service_role JWT to a named secret API key.
CREATE SCHEMA IF NOT EXISTS private;
REVOKE ALL ON SCHEMA private FROM PUBLIC, anon, authenticated;

CREATE OR REPLACE FUNCTION private.get_reminders_secret_key()
RETURNS text
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    WHERE name = 'reminders_secret_key'
    LIMIT 1;
    RETURN key_val;
...
END;
$$;
REVOKE ALL ON FUNCTION private.get_reminders_secret_key() FROM PUBLIC, anon, authenticated;
...
DROP FUNCTION IF EXISTS private.get_service_role_key();
