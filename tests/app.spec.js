...
test.describe('Privyetik E2E Test Suite', () => {
  ...
  test('Shared text opens the Add Word modal and consumes the URL parameter', async ({ page }) => {
    await page.goto('/?add_word=%D0%BA%D0%BD%D0%B8%D0%B3%D0%B0');

    await expect(page.locator('#modal-add-word')).toHaveClass(/active/);
    await expect(page.locator('#add-word-input')).toHaveValue('книга');
    await expect.poll(() => new URL(page.url()).searchParams.has('add_word')).toBe(false);
  });
