-- Keep reminders enabled for the owner's account even on days when they
-- already studied. All other users retain the normal skip-after-study rule.
...
BEGIN
    ...
    FOR r IN
        SELECT
            ...
            s.streak,
            lower(u.email) = 'itayura@gmail.com' AS ignore_study_suppression
    ...
    LOOP
        ...
        -- Remind the owner even after studying; all other users are skipped
        -- once their local last_active_date matches today.
        IF current_time_in_tz >= reminder_time
           AND (
               r.ignore_study_suppression
               OR r.last_active_date IS NULL
               OR r.last_active_date <> today_in_tz
           )
           AND NOT EXISTS (
